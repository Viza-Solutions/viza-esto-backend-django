...
class ExcelDownloadView(APIView):
    ...
    def get(self, request):
        ...
        header_row = ['Name', 'Client', 'Added By', 'Country', 'Town', 'Address', 'Description', 'Rooms', 'Deleted', 'Status', 'Created At', 'Updated At']
        worksheet.append(header_row)

        # Write data to the Excel file.
        for item in serializer.data:
            row = [item['name'], item['client'], item['added_by'], item['country'], item['town'], item['address'], item['description'], item['rooms'], item['deleted'], item['status'], item['created_at'], item['updated_at']]
